...
namespace Infrastructure.EventStore.Repository
{
        ...
        where T : class, IAggregate
    {
        ...
        Task CreateAsync(Guid id, IEnumerable<object> @events, CancellationToken ct = default);
        Task AppendAsync(Guid id, object @event, CancellationToken ct = default);
        Task AppendAsync(Guid id, IEnumerable<object> @events, CancellationToken ct = default);
        Task<T?> FindOneAsync(
            Guid id,
